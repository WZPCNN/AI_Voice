...
AIVoice monorepo 当前文档体系薄弱:根 README 目录结构严重过时(标注 `engine/`/`planner/`/`pgvector`,实际为 `worker.py`/`orchestrator.py`/`Qdrant`;只文档化 3 种模式,实际 [types/index.ts:166](file:///d:\MyProject\AIVoice\packages\shared\src\types\index.ts#L166) 定义 5 种 AppMode),4 个子包无 README,无 ARCHITECTURE/CONTRIBUTING/CHANGELOG。同时无任何文档工具(Storybook/TypeDoc/Compodoc/MkDocs),CI 流水线虽成熟(GitHub Actions 6 job + changesets 已集成)但无文档构建/部署。
...
- **agents/src/ 实际结构**:顶层 `worker.py`/`runner.py`/`orchestrator.py`/`decomposer.py`/`executor.py`/`tools.py` + 子包 `memory/`/`skills/`/`mcp_client/`/`prompts/`/`models/`(README 的 `engine/`/`planner/`/`multi_agent/` 已不存在)
...
| 文件 | 操作 | 关键内容 |
|------|------|----------|
| [README.md](file:///d:\MyProject\AIVoice\README.md) | 改 | 修目录结构(agents/src 实际)、3→5 种模式、技术栈版本(pgvector→Qdrant、React 18→19)、新增"文档"章节(链接 Pages 各子站 + 本地命令)、新增"贡献"指向 CONTRIBUTING |
| `ARCHITECTURE.md` | 新建 | 全链路数据流(web→api-server→Redis→worker→LLM)、组件职责(web/api-server/agents/shared)、基础设施(PG+Qdrant+Redis)、5 种模式详解、数据模型对应 |
...
| `apps/web/README.md` | 新建 | 概述(React 19+Vite 8+Tailwind 4)、目录结构(chat/10 组件)、本地开发(端口 3000,代理 /api→4000)、Storybook 启动、核心组件清单 |
| `apps/api-server/README.md` | 新建 | 概述(NestJS 11+Prisma 7+Socket.io)、7 模块表、本地开发(prisma:generate→pnpm dev,端口 4000)、API 文档链接、环境变量 |
...
| `agents/README.md` | 新建 | 概述(Python 3.12+LangGraph+uv)、目录结构、本地开发(uv sync/pytest/worker 启动)、MkDocs 文档命令、内置技能 |
...
**修改**:`apps/web/package.json` devDependencies 加 `storybook@^9.1`、`@storybook/react-vite@^9.1`、`@storybook/addon-themes@^9.1`;scripts 加 `storybook`(dev -p 6006)、`build-storybook`(build -o storybook-static)、`docs`(=build-storybook)

**关键**:不装 `@storybook/addon-essentials`(Storybook 9 已将 controls/actions/viewport/docs 并入核心)
...
**修改**:两个包 package.json devDependencies 加 `typedoc@^0.28.19`(0.28.18+ 支持 TS 6.0)、`typedoc-plugin-markdown@^4`(可选);scripts 加 `docs:typedoc`(=typedoc)、`docs`

### 4. Compodoc 集成(apps/api-server)

**修改**:`apps/api-server/package.json` devDependencies 加 `@compodoc/compodoc@^1.2.1`(若 TS 6.0 失败改 `^2.0.0`);scripts 加 `docs:compodoc`(`compodoc -p tsconfig.json -d docs/compodoc`);`docs` 改为 `pnpm docs:typedoc && pnpm docs:compodoc`
...
**新建文件**:
- `agents/mkdocs.yml` — site_name: Agent Engine、theme: material(暗/亮切换+tabs)、plugins: mkdocstrings(`docstring_style: google` 匹配 [registry.py](file:///d:\MyProject\AIVoice\agents\src\skills\registry.py) 的 Args/Returns)、nav(首页/架构/API 参考)
